...
mapbinHex = mapbinHex[0:16]  # 切掉文件头
print(mapbinHex)
isExists = os.path.exists(Path(gameName))
if not isExists:  # 判断如果文件不存在,则创建
    os.makedirs(Path(gameName))
f = open(Path(gameName + '/' + 'Map.bin'), mode='wb')
f.write(mapBin)
f.close()

f = open(Path(gameName + '/' + 'Map2.txt'), mode='wb')
mapbinHex = base64.b16encode(mapBin).decode()
...
# mapbinHex = re.sub('4FFEFA0220000000(?!.(..)*$)', '0D0A', mapbinHex)
mapbinHex_bytes = mapbinHex.encode()
# for i in range(0, len(mapbinHex_bytes), 16):
#     chunk = mapbinHex_bytes[i:i+16]
#     f.write(chunk)
#     f.write(b'\n')
f.write(mapbinHex_bytes)
f.close()
os._exit(0)
